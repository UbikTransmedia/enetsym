...
try:
    ...
    def dibujar_red():
        ...
        for node in G.nodes():
            ...
            if tipo == 'suministrador':
                ...
                carga_max = G.nodes[node]['carga_max']
                carga_relativa = carga_actual / carga_max if carga_max > 0 else 0
                # Color entre negro (0% carga) y verde (100% carga)
        ...
        nx.draw_networkx_edges(G, pos, ax=ax1, alpha=0.3, width=0.5, edge_color='white')
        ...
        ax2_right.set_facecolor('#2B2B2B')
        
        ax2.plot(tiempos, serie_estaciones_cero, color='lightgreen', label='Estaciones al 0%')
        ax2.plot(tiempos, serie_estaciones_max, color='green', label='Estaciones al 100%')
        ax2.plot(tiempos, serie_carga_media_estaciones, color='orange', label='Carga media estaciones')
        ax2.plot(tiempos, serie_produccion_media_suministradores, color='pink', label='Producción media suministradores')
        ...
        ax2_right.plot(tiempos, serie_total_energia_acumulada, color='yellow', label='Total energía acumulada')
        
        # Mostrar el último valor de cada serie sobre el último punto
        if tiempos:
            last_time = tiempos[-1]
            ax2.text(last_time, serie_estaciones_cero[-1], f'{serie_estaciones_cero[-1]}', color='lightgreen')
            ax2.text(last_time, serie_estaciones_max[-1], f'{serie_estaciones_max[-1]}', color='green')
            ax2.text(last_time, serie_carga_media_estaciones[-1], f'{serie_carga_media_estaciones[-1]:.2f}', color='orange')
            ax2.text(last_time, serie_produccion_media_suministradores[-1], f'{serie_produccion_media_suministradores[-1]:.2f}', color='pink')
            ...
            ax2_right.text(last_time, serie_total_energia_acumulada[-1], f'{serie_total_energia_acumulada[-1]:.1f}', color='yellow')
        
        # Leyendas y etiquetas
        ax2.set_xlabel('Tiempo', color='white')
        ax2.set_ylabel('Valores', color='white')
        ax2_right.set_ylabel('Energía Acumulada', color='yellow')
        ...
        ax2_right.tick_params(axis='y', colors='yellow')
        ax2.xaxis.label.set_color('white')
        ax2.yaxis.label.set_color('white')
        ax2_right.yaxis.label.set_color('yellow')
        ...
        ax2.grid(True, color='gray', linestyle='dashed', linewidth=0.5)
    ...
    fig.patch.set_facecolor('#2B2B2B')  # Fondo gris oscuro

    # Dividir la figura en dos ejes
    ax1 = fig.add_axes([0.01, 0.35, 0.98, 0.64])  # [left, bottom, width, height]
    ax2 = fig.add_axes([0.05, 0.05, 0.9, 0.25], facecolor='#2B2B2B')
    ax2_right = ax2.twinx()
    ...
    time_label = tk.Label(root, text="Unidad de tiempo: 0", font=("Arial", 12), anchor='w', justify='left', fg='white', bg='#2B2B2B')
