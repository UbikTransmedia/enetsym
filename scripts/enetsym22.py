...
try:
    ...
    def clasificar_nodos(G):
        ...
        emojis_energia = {
            'EÓLICA': '💨',        # Ráfagas de viento
            'SOLAR': '☀️',
            'GEOTÉRMICA': '🔥',    # Fuego
            'NUCLEAR': '⚛️',
            'OCEÁNICA': '🌊',
            'HIDROELÉCTRICA': '💦', # Gotas de agua
            'BIOMASA': '🌿'        # Hierba
        }
    ...
    def dibujar_red():
        ...
        for node in G.nodes():
            ...
            if tipo == 'suministrador':
                ...
                carga_max = G.nodes[node]['carga_max']
                carga_relativa = carga_actual / carga_max if carga_max > 0 else 0
                size = (100 + 200 * carga_relativa) * 3  # Tamaño depende de la carga
                # Color que varía de gris a amarillo
                color_low = (0.5, 0.5, 0.5)  # Gris
                color_high = (1.0, 1.0, 0.0)  # Amarillo
        ...
        if fases_solares:
            ...
            time_of_day_label.config(text="Hora: --:--")
    ...
    time_label = tk.Label(root, text="Unidad de tiempo: 0\nTiempo transcurrido: 0.00 s", font=("Arial", 12), anchor='w', justify='left')
