...
try:
    ...
    def dibujar_red():
        ...
        for node in G.nodes():
            ...
            if tipo == 'suministrador':
                ...
                size = 100 + 200 * carga_relativa  # El tamaño depende de la carga
            elif tipo == 'estacion':
                carga_actual = G.nodes[node]['carga_actual']
                carga_max = G.nodes[node]['carga_max']
                carga_relativa = carga_actual / carga_max if carga_max > 0 else 0
                ...
                color_intensity = min(max(energia_neta / 50, 0), 1)
    ...
    def actualizar():
        ...
        for node in G.nodes():
            if G.nodes[node]['tipo'] == 'distribuidor':
                # Recibir energía de suministradores
                ...
                # Enviar energía a estaciones
                vecinos = list(G.neighbors(node))
                estaciones_vecinas = [n for n in vecinos if G.nodes[n]['tipo'] == 'estacion']
                if estaciones_vecinas:
                    carga_por_estacion = G.nodes[node]['carga_actual'] / len(estaciones_vecinas)
    ...
    # Generar la red
    seed = 42
    num_nodos = 500  # Puedes ajustar este valor
